...
void *SparseBase_malloc_onnode    /* pointer to allocated block of memory */
(
    size_t nitems,          /* number of items to malloc */
    size_t size_of_item,     /* sizeof each item */
    size_t const numa_node_rank
)
{
    void *p ;
    size_t size ;
    ...
    {
        p = (void *) TPSM_Numa_MallocOnNode (size, numa_node_rank);
        //p = (void *) (SparseBase_config.malloc_func) (size) ;
    }
    return (p) ;
}
...
// numa 亲和的realloc
void *SparseBase_numa_realloc   /* pointer to reallocated block of memory, or
                               to original block if the realloc failed. */
(
    size_t nitems_new,      /* new number of items in the object */
    size_t nitems_old,      /* old number of items in the object */
...
)
{
    ...
    size_old = nitems_old * size_of_item  ;
    size_new = nitems_new * size_of_item  ;

    if (size_new != ((double) nitems_new) * size_of_item)
    {
        /* size_t overflow */
        (*ok) = 0 ;
    }
    ...
    {
        /* change the size_new of the object from nitems_old to nitems_new */
        void *pnew ;
        pnew = (void *) TPSM_Numa_Realloc (p, size_old, size_new) ;
        
        if (pnew == NULL)
        {
            if (nitems_new < nitems_old)
            {
                /* the attempt to reduce the size_new of the block failed, but
                   the old block is unchanged.  So pretend to succeed. */
                (*ok) = 1 ;
            }
            else
...
void *SparseBase_numa_free      /* always returns NULL */
(
    void *p,                /* block to free */
    size_t const bytes
)
