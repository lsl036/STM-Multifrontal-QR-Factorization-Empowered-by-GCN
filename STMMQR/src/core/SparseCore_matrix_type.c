 ...
 * 
 */
sparse_csc *SparseCore_allocate_sparse
(
    /* ---- input ---- */
    size_t nrow,	/* A的行数 */
    size_t ncol,	/* A的列数 */
...
)
{
    ...
    /* 确保维度不会导致整型溢出 */
    (void) SparseCore_add_size_t (ncol, 2, &ok) ;
    ...
    A = SparseCore_malloc (sizeof (sparse_csc), 1, Common) ;
    ...
    A->ncol = ncol ;
    A->nzmax = nzmax ;
    A->packed = packed ;    /* 默认是压缩格式(A->nz不存在) */
    A->stype = stype ;
    A->itype = ITYPE ;
    A->xtype = xtype ;
    A->dtype = DTYPE ;
    ...
    /* 分配大小为O(ncol)的空间 */
    A->p = SparseCore_malloc (((size_t) ncol)+1, sizeof (Int), Common) ;
    if (!packed) // 没走这里
    {
	A->nz = SparseCore_malloc (ncol, sizeof (Int), Common) ;
    }

    /* 分配大小为O(nz) 的空间*/
    nzmax0 = 0 ;
    SparseCore_realloc_multiple (nzmax, 1, xtype, &(A->i), NULL, &(A->x), &(A->z),
	    &nzmax0, Common) ;
...
)
{
    ...
    n = A->ncol ;
    nz = A->nzmax ;
    A->p  = SparseCore_free (n+1, sizeof (Int), A->p,  Common) ;
    A->i  = SparseCore_free (nz,  sizeof (Int), A->i,  Common) ;
    A->nz = SparseCore_free (n,   sizeof (Int), A->nz, Common) ;

    switch (A->xtype)
    {
	case SPARSE_REAL:
	    A->x = SparseCore_free (nz, sizeof (double), A->x,  Common) ;
	    break ;
    }

    *AHandle = SparseCore_free (1, sizeof (sparse_csc), (*AHandle), Common) ;
    return (TRUE) ;
}
 ...
 * 
 */
int SparseCore_reallocate_sparse
(
    /* ---- input ---- */
    size_t nznew,	    /* A中条目的新# */
    /* ---- in/out --- */
...
)
{
    ...
    SparseCore_realloc_multiple (MAX (1,nznew), 1, A->xtype, &(A->i), NULL,
...
)
{
    ...
    n = MIN (nrow, ncol) ;
    A = SparseCore_allocate_sparse (nrow, ncol, n, TRUE, TRUE, 0, xtype,
	    Common) ;
...
)
{
    ...
    return ( SparseCore_allocate_sparse (nrow, ncol, nzmax, TRUE, TRUE, 0, xtype,
	    Common)) ;
}
 ...
 * 
 */
sparse_csc *SparseCore_copy_sparse
(
    /* ---- input ---- */
...
)
{
    ...
    C = SparseCore_allocate_sparse (A->nrow, A->ncol, A->nzmax, A->sorted,
 ...
 * 
 */
dense_array *SparseCore_allocate_dense
(
    /* ---- input ---- */
    size_t nrow,	/* 矩阵的行数 */
    size_t ncol,	/* 矩阵的列数 */
...
)
{
    ...
    /* nzmax = MAX (1, d*ncol) ; */
    nzmax = SparseCore_mult_size_t (d, ncol, &ok) ;
    nzmax = MAX (1, nzmax) ;
    ...
    X = SparseCore_malloc (sizeof (dense_array), 1, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (NULL) ;	    /* 内存溢出 */
    }
    ...
    nzmax0 = 0 ;
    SparseCore_realloc_multiple (nzmax, 0, xtype, NULL, NULL, &(X->x), &(X->z),
	    &nzmax0, Common) ;
...
)
{
    ...
    RETURN_IF_NULL_COMMON (NULL) ;
    X = SparseCore_allocate_dense (nrow, ncol, nrow, xtype, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (NULL) ;	    /* 检查错误 */
    }
...
)
{
    ...
    {
	case SPARSE_REAL:
	    X->x = SparseCore_free (X->nzmax, sizeof (double), X->x, Common) ;
	    break ;
    }

    *XHandle = SparseCore_free (1, sizeof (dense_array), (*XHandle), Common) ;
    return (TRUE) ;
}
 ...
 */
dense_array *SparseCore_ensure_dense
(
    /* ---- input/output ---- */
...
)
{
        ...
        || X->d != d || X->xtype != xtype)
    {
         ...
         * 设置错误，并将X返回为NULL。 */
        SparseCore_free_dense (XHandle, Common) ;
        X = SparseCore_allocate_dense (nrow, ncol, d, xtype, Common) ;
        *XHandle = X ;
    }
    return (X) ;
}
 ...
 *          支持所有的xtype。
 */
dense_array *SparseCore_sparse_to_dense
(
    /* ---- input ---- */
 ...
 * @brief 将稠密矩阵转换为稀疏矩阵
 */
sparse_csc *SparseCore_dense_to_sparse
(
    /* ---- input ---- */
 ...
 * 
 */
dense_array *SparseCore_copy_dense
(
    /* ---- input ---- */
    dense_array *X,	/* 需要拷贝的矩阵 */
    /* --------------- */
...
)
{
    ...
    Y = SparseCore_allocate_dense (X->nrow, X->ncol, X->d, X->xtype, Common) ;
    if (Common->status < SPARSE_OK)
    {
    ...
    SparseCore_copy_dense2 (X, Y, Common) ;
 ...
 *          不需要额外工作空间
 */
sparse_triplet *SparseCore_allocate_triplet
(
    /* ---- input ---- */
    size_t nrow,	/* T的行数 */
    size_t ncol,	/* T的列数 */
...
)
{
    ...
    T = SparseCore_malloc (sizeof (sparse_triplet), 1, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (NULL) ;	    /* 内存溢出 */
    }
    ...
    nzmax0 = 0 ;
    SparseCore_realloc_multiple (nzmax, 2, xtype, &(T->i), &(T->j),
...
)
{
    ...
    nz = T->nzmax ;
    T->j = SparseCore_free (nz, sizeof (Int), T->j, Common) ;
    T->i = SparseCore_free (nz, sizeof (Int), T->i, Common) ;
    if (T->xtype == SPARSE_REAL)
    {
	T->x = SparseCore_free (nz, sizeof (double), T->x, Common) ;
    }
    *THandle = SparseCore_free (1, sizeof (sparse_triplet), (*THandle), Common) ;
    return (TRUE) ;
}
 ...
 */
int SparseCore_reallocate_triplet
(
    /* ---- input ---- */
    size_t nznew,	    /* T中的新条目# */
    /* ---- in/out --- */
...
)
{
    ...
    SparseCore_realloc_multiple (MAX (1,nznew), 2, T->xtype, &(T->i), &(T->j),
 ...
 * 
 */
sparse_csc *SparseCore_triplet_to_sparse
(
    /* ---- input ---- */
...
)
{
    ...
    R = SparseCore_allocate_sparse (ncol, nrow, nz, FALSE, FALSE, -stype,
	    T->xtype, Common) ;
    ...
    {
	/* 三元组矩阵非法 */
	SparseCore_free_sparse (&R, Common) ;
	return (NULL) ;
    }
    ...
    A = SparseCore_allocate_sparse (nrow, ncol, MAX (anz, nzmax), TRUE, TRUE,
	stype, T->xtype, Common) ;

    if (stype)
    {
	ok = SparseCore_transpose_sym (R, 1, NULL, A, Common) ;
    }
    else
    {
	ok = SparseCore_transpose_unsym (R, 1, NULL, NULL, 0, A, Common) ; 
    }
 ...
 *          不需要额外的工作空间
 */
sparse_triplet *SparseCore_sparse_to_triplet
(
    /* ---- input ---- */
    sparse_csc *A,	/* 需要拷贝的矩阵 */
    /* --------------- */
...
)
{
    ...
    nz = SparseCore_nnz (A, Common) ;
    T = SparseCore_allocate_triplet (nrow, ncol, nz, A->stype, A->xtype, Common) ;
 ...
 */
sparse_triplet *SparseCore_copy_triplet
(
    /* ---- input ---- */
    sparse_triplet *T,	/* 需要拷贝的矩阵 */
    /* --------------- */
...
)
{
    ...
    C = SparseCore_allocate_triplet (T->nrow, T->ncol, T->nzmax, T->stype,
	    xtype, Common) ;
 ...
 */
sparse_factor *SparseCore_allocate_factor
(
    /* ---- input ---- */
    size_t n,		/* L为n*n */
    /* --------------- */
...
)
{
    ...
    /* 确保维度不会导致整型溢出 */
    (void) SparseCore_add_size_t (n, 2, &ok) ;
    if (!ok || n > Int_max)
    {
	ERROR (SPARSE_TOO_LARGE, "problem too large") ;
	return (NULL) ;
    ...
    L = SparseCore_malloc (sizeof (sparse_factor), 1, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (NULL) ;	    /* 内存溢出 */
    }
    ...
    L->Perm = SparseCore_malloc (n, sizeof (Int), Common) ;
    L->IPerm = NULL ;       /* 仅在需要时由cholmod_solve2创建 */
    L->ColCount = SparseCore_malloc (n, sizeof (Int), Common) ;
...
)
{
    ...
    /* L的符号部分 */
    SparseCore_free (n,   sizeof (Int), L->Perm,     Common) ;
    SparseCore_free (n,   sizeof (Int), L->IPerm,    Common) ;
    SparseCore_free (n,   sizeof (Int), L->ColCount, Common) ;

    /* L的simplicial形式 */
    SparseCore_free (n+1, sizeof (Int), L->p,        Common) ;
    SparseCore_free (lnz, sizeof (Int), L->i,        Common) ;
    SparseCore_free (n,   sizeof (Int), L->nz,       Common) ;
    SparseCore_free (n+2, sizeof (Int), L->next,     Common) ;
    SparseCore_free (n+2, sizeof (Int), L->prev,     Common) ;

    /* L的超节点形式 */
    SparseCore_free (s,   sizeof (Int), L->pi,       Common) ;
    SparseCore_free (s,   sizeof (Int), L->px,       Common) ;
    SparseCore_free (s,   sizeof (Int), L->super,    Common) ;
    SparseCore_free (ss,  sizeof (Int), L->s,        Common) ;

    /* simplicial和超节点L的数值 */
    if (L->xtype == SPARSE_REAL)
    {
	SparseCore_free (xs, sizeof (double), L->x, Common) ;
    }
    *LHandle = SparseCore_free (1, sizeof (sparse_factor), (*LHandle), Common) ;
    return (TRUE) ;
}
 ...
 *          不需要额外工作空间
 */
int SparseCore_reallocate_factor
(
    /* ---- input ---- */
    size_t nznew,	    /* L中条目的新# */
    /* ---- in/out --- */
...
)
{
    ...
    SparseCore_realloc_multiple (nznew, 1, L->xtype, &(L->i), NULL,
 ...
 *          不需要额外的工作空间
 */
int SparseCore_reallocate_column
(
    /* ---- input ---- */
    size_t j,		    /* 要重新分配的列 */
...
)
{
	...
	if (xneed > Size_max ||
		!SparseCore_reallocate_factor ((Int) xneed, L, Common))
	{
	    /* 内存溢出, 转换为简单的符号 */
	    SparseCore_change_factor (SPARSE_PATTERN, L->is_ll, FALSE, TRUE,
		    TRUE, L, Common) ;
	    ERROR (SPARSE_OUT_OF_MEMORY, "内存溢出; L now symbolic") ;
	    return (FALSE) ;	    /* 内存溢出 */
	}
	/* 填充所有列，以便每个列最多拥有grow2空闲空间 */
	SparseCore_pack_factor (L, Common) ;
 ...
 */
sparse_csc *SparseCore_factor_to_sparse
(
    /* ---- in/out --- */
...
)
{
    ...
    /* 作为LL或者LDL'留下 */
    if (!SparseCore_change_factor (L->xtype, L->is_ll, FALSE, TRUE, TRUE, L,
		Common))
    {
    ...
    Lsparse = SparseCore_malloc (sizeof (sparse_csc), 1, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (NULL) ;		/* 内存溢出 */
    }
    ...
    L->z = NULL ;
    L->xtype = SPARSE_PATTERN ;
    SparseCore_change_factor (SPARSE_PATTERN, FALSE, FALSE, TRUE, TRUE, L,
	    Common) ;
 ...
 */
sparse_factor *SparseCore_copy_factor
(
    /* ---- input ---- */
...
)
{
	...
	*L2pi, *L2px, *L2s ;
    ...
    L2 = SparseCore_allocate_factor (n, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (NULL) ;	    /* 内存溢出 */
    }
	...
	L2->nzmax = L->nzmax ;
	if (!SparseCore_change_factor (L->xtype, L->is_ll, FALSE, -1, TRUE,
		    L2, Common))
	{
	    SparseCore_free_factor (&L2, Common) ;
	    return (NULL) ;	/* 内存溢出 */
	}
	...
	if (!SparseCore_change_factor (L->xtype, TRUE, TRUE, TRUE, TRUE, L2,
		    Common))
	{
	    SparseCore_free_factor (&L2, Common) ;
	    return (NULL) ;	/* 内存溢出 */
