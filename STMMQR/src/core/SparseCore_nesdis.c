/*
 * @file SparseCore_nesdis.c
 * @author your name (you@domain.com)
 * @brief 嵌套分割和图划分。
 * 
 * SparseCore_bisect: 找到分割图成为两个部分的结点，并与之前的图做对比。
 * 
 * SparseCore_nested_dissection: 嵌套分割算法，相比于METIS_NodeND
 *                               可以给出5%到10%更少的非零元。
 * 
 * SparseCore__collapse_septree: 修剪 nested_dissection 返回的树.
 * @version 0.1
 * @date 2020-11-18
 * 
...
/* === partition ============================================================ */
/* ========================================================================== */
static Sparse_long partition    /* size of separator or -1 if failure */
(
...
)
{
	...
	csep = SparseCore_metis_bisector (C, Cnw, Cew, Part, Common) ;
...
static Sparse_long clear_flag (Int *Map, Int cn, sparse_common *Common)
{
    Int nrow, i ;
    Int *Flag ;
    // PRINT2 (("old mark %ld\n", Common->mark)) ;
...
)
{
    ...
    /* s = n + MAX (n, A->ncol) */
    s = SparseCore_add_size_t (A->nrow, MAX (A->nrow, A->ncol), &ok) ;
    if (!ok)
    {
    ...
    Bew = SparseCore_malloc (csize, sizeof (Int), Common) ;
    if (Common->status < SPARSE_OK)
    {
	/* out of memory */
	SparseCore_free_sparse (&B, Common) ;
	SparseCore_free (csize, sizeof (Int), Bew, Common) ;
	return (EMPTY) ;
    }
...
Sparse_long SparseCore_nested_dissection
...
)
{
	...
	*Work3n ;
    ...
    Work3n = SparseCore_malloc (n, 3*sizeof (Int), Common) ;
    Part = Work3n ;		/* size n */
    Bnw  = Part + n ;		/* size n */
    Cnw  = Bnw + n ;		/* size n */
    ...
    if (Common->status < SPARSE_OK)
    {
	SparseCore_free (3*n, sizeof (Int), Work3n, Common) ;
	return (EMPTY) ;
    }
    Bp = B->p ;
    Bi = B->i ;
    bnz = SparseCore_nnz (B, Common) ;
    // ASSERT ((Int) (B->nrow) == n && (Int) (B->ncol) == n) ;
    csize = MAX (n, bnz) ;
    ...
    C = SparseCore_allocate_sparse (n, n, csize, FALSE, TRUE, 0, SPARSE_PATTERN,
	    Common) ;
    Cew  = SparseCore_malloc (csize, sizeof (Int), Common) ;
	...
	SparseCore_free_sparse (&B, Common) ;
	SparseCore_free (csize, sizeof (Int), Cew, Common) ;
	SparseCore_free (3*n, sizeof (Int), Work3n, Common) ;
	Common->mark = EMPTY ;
	SPARSE_CLEAR_FLAG (Common) ;
    ...
    ncomponents = SparseChol_postorder (CParent, n, NULL, Post, Common) ;
	...
	if (A->stype != 0)
	{
		...
		ok = SparseCore_csymamd (B, Cmember, Perm, Common) ;
	    }
	    else
	    {
		/* workspace: Head (nrow), Iwork (4*nrow) */
		ok = SparseCore_camd (B, NULL, 0, Cmember, Perm, Common) ;
	    }
	    SparseCore_free_sparse (&B, Common) ;
	    if (!ok)
	    {
	...
	else
	{
	    ...
	    if (!SparseCore_ccolamd (A, fset, fsize, Cmember, Perm, Common))
	    {
		/* ccolamd failed */
...
)
{
    ...
    if (n < ncomponents)
    {
	ERROR (SPARSE_INVALID, "invalid separator tree") ;
	return (EMPTY) ;
    }
    Common->status = SPARSE_OK ;
    nc = ncomponents ;
    ...
    /* s = 3*ncomponents */
    s = SparseCore_mult_size_t (ncomponents, 3, &ok) ;
    if (!ok)
    {
	ERROR (SPARSE_TOO_LARGE, "problem too large") ;
	return (EMPTY) ;
