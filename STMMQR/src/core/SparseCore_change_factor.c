...
)
{
	...
	/* 释放旧的L->i和L->x，并更换新的 */
	SparseCore_free (L->nzmax, sizeof (Int), L->i, Common) ;

#ifdef REAL
	SparseCore_free (L->nzmax, sizeof (double), L->x, Common) ;
#else
	SparseCore_free (L->nzmax, sizeof (double), L->x, Common) ;
	SparseCore_free (L->nzmax, sizeof (double), L->z, Common) ;
#endif
...
)
{
    ...
    Lp = SparseCore_malloc (n1, sizeof (Int), Common) ;
    Lnz = SparseCore_malloc (n, sizeof (Int), Common) ;
    Lprev = SparseCore_malloc (n2, sizeof (Int), Common) ;
    Lnext = SparseCore_malloc (n2, sizeof (Int), Common) ;

    if (Common->status < SPARSE_OK)
    {
	SparseCore_free (n1, sizeof (Int), Lp,    Common) ;
	SparseCore_free (n,   sizeof (Int), Lnz,   Common) ;
	SparseCore_free (n2, sizeof (Int), Lprev, Common) ;
	SparseCore_free (n2, sizeof (Int), Lnext, Common) ;
	return (FALSE) ;	/* 内存溢出 */
    }
...
)
{
    ...
    Lsuper = SparseCore_malloc (nsuper1, sizeof (Int), Common) ;
    Lpi    = SparseCore_malloc (nsuper1, sizeof (Int), Common) ;
    Lpx    = SparseCore_malloc (nsuper1, sizeof (Int), Common) ;

    /* O(ssize)大小的数组, ssize <= nnz(L), 并且一般可能会更小 */
    Ls = SparseCore_malloc (ssize, sizeof (Int), Common) ;

    if (Common->status < SPARSE_OK)
    {
	SparseCore_free (nsuper1, sizeof (Int), Lsuper, Common) ;
	SparseCore_free (nsuper1, sizeof (Int), Lpi,    Common) ;
	SparseCore_free (nsuper1, sizeof (Int), Lpx,    Common) ;
	SparseCore_free (ssize,    sizeof (Int), Ls,     Common) ;
	return (FALSE) ;	/* 内存溢出 */
    }
...
)
{
    ...
    L->p     = SparseCore_free (n1,  sizeof (Int),      L->p,     Common) ;
    L->i     = SparseCore_free (lnz, sizeof (Int),      L->i,     Common) ;
    L->x     = SparseCore_free (xs,  e*sizeof (double), L->x,     Common) ;
    L->z     = SparseCore_free (lnz, sizeof (double),   L->z,     Common) ;
    L->nz    = SparseCore_free (n,   sizeof (Int),      L->nz,    Common) ;
    L->next  = SparseCore_free (n2,  sizeof (Int),      L->next,  Common) ;
    L->prev  = SparseCore_free (n2,  sizeof (Int),      L->prev,  Common) ;
    L->super = SparseCore_free (s,   sizeof (Int),      L->super, Common) ;
    L->pi    = SparseCore_free (s,   sizeof (Int),      L->pi,    Common) ;
    L->px    = SparseCore_free (s,   sizeof (Int),      L->px,    Common) ;
    L->s     = SparseCore_free (ss,  sizeof (Int),      L->s,     Common) ;
    L->nzmax = 0 ;
    L->is_super = FALSE ;
    L->xtype = SPARSE_PATTERN ;
    L->dtype = DTYPE ;
...
)
{
    ...
    /* 释放除超节点数值因子以外的所有因子 */
    L->x = SparseCore_free (L->xsize, sizeof (double), L->x, Common) ;
    L->xtype = SPARSE_PATTERN ;
    L->dtype = DTYPE ;
    L->minor = L->n ;
    L->is_ll = TRUE ;	    /* 不支持超节点LDL */
...
)
{
    ...
    /* 指定 L->i, L->x, L->z */
    e = 1 ;
    if (!ok || ! SparseCore_realloc_multiple (lnz, 1, to_xtype, &(L->i), NULL,
		&(L->x), &(L->z), &(L->nzmax), Common))
    {
	L->p    = SparseCore_free (n+1, sizeof (Int),      L->p, Common) ;
	L->nz   = SparseCore_free (n,   sizeof (Int),      L->nz, Common) ;
	L->prev = SparseCore_free (n+2, sizeof (Int),      L->prev, Common) ;
	L->next = SparseCore_free (n+2, sizeof (Int),      L->next, Common) ;
	L->i    = SparseCore_free (lnz, sizeof (Int),      L->i, Common) ;
	L->x    = SparseCore_free (lnz, e*sizeof (double), L->x, Common) ;
	L->z    = SparseCore_free (lnz, sizeof (double),   L->z, Common) ;
	return ;	/* 内存溢出 */
    }
...
)
{
	...
	SparseCore_realloc_multiple (lnz, 1, L->xtype, &newLi, NULL,
...
)
{
    ...
    Li = SparseCore_malloc (lnz, sizeof (Int), Common) ;
    if (Common->status < SPARSE_OK)
    {
	return ;	/* 内存溢出 */
    }
    ...
    if (!allocate_simplicial_numeric (L, Common))
    {
	SparseCore_free (lnz, sizeof (Int), Li, Common) ;
	return ;	/* 内存溢出 */
    }
    ...
    L->super = SparseCore_free (nsuper+1, sizeof (Int), L->super, Common) ;
    L->pi    = SparseCore_free (nsuper+1, sizeof (Int), L->pi, Common) ;
    L->px    = SparseCore_free (nsuper+1, sizeof (Int), L->px, Common) ;
    L->s     = SparseCore_free (L->ssize, sizeof (Int), L->s, Common) ;
...
)
{
    ...
    Lx = SparseCore_malloc (L->xsize, wentry * sizeof (double), Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (FALSE) ;	/* 内存溢出 */
    }
