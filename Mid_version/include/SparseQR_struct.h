...
typedef struct qr_numeric_struct
{
    ...
    double **Rblock ;    // 大小为nf。R[f] 是一个(double *)指针指向 R块中的波前阵F
                        //  它是一个大小为 Rm(f)-by-Rn(f) 的上梯形，但仅
                        //     以列压缩格式保存上三角部分。

    double **Stacks ;    //     大小为ns。一个堆栈数组，保存 R 和 H 因子以及当前在顶部的波前矩阵F
                        //  接下来是空区域，然后是底部波前矩阵的贡献块C。当分解完成后，只有
                        //   在栈顶的 R 和 H 部分保留。
    ...
    Sparse_long n ;            
    Sparse_long m ;
    Sparse_long nf ;            //    波前矩阵 数目
    Sparse_long ntasks ;        //  任务图中实际上使用的tasks
    Sparse_long ns ;            //   栈 数目
    Sparse_long maxstack ;      //   顺序 栈的大小，如果使用
    ...
    Sparse_long *HStair ;   //    staircase是每列中最后一个非零项的行索引
                        //     数组 Hstair[Rp [f] ...Rp [f+1]-1 ] 给出了波前阵F的 staircase

    double *HTau ;       //  数组HTau [Rp [f] ... Rp [f+1]-1] 给出 波前矩阵F的 Householder系数

    Sparse_long *Hii ;      //   H的行索引

    Sparse_long *HPinv ;    //   如果A和H 的 行i 是 R的行k，则 HPinv[i] = k 
                        //    这个排序包含了 QRsym->PLinv，
                        //     以及在分解过程中通过关键行排序 构造的置换。
...
typedef struct SparseQR_factorization_struct
{
    ...
    Sparse_long *Rmap ;     //  Rmap大小为n。 如果R的第j列是第k个活列，且k < QR->rank，
