...
)
{
    ...
    diag = (mode >= 0) ;
    n = A->nrow ;
    CORE(allocate_work) (n, MAX (A->ncol, A->nrow), values ? n : 0, Common) ;
    ...
    F = CORE(ptranspose) (A, values, NULL, fset, fsize, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (NULL) ;	    /* 内存溢出 */
    }
    ...
    C = CORE(allocate_sparse) (n, n, cnz + extra, FALSE, TRUE, 0,
...
)
{
    ...
    nrow = A->nrow ;
    ncol = A->ncol ;
    CORE(allocate_work) (nrow, MAX (nrow,ncol), values ? nrow : 0, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (NULL) ;	    /* 内存溢出 */
    }
	...
	if (A->stype)
	{
	    ...
	    A2 = CORE(copy) (A, 0, values, Common) ;
	    if (Common->status < SPARSE_OK)
	    {
		return (NULL) ;	    /* 内存溢出 */
	    }
	...
	{
	    /* 工作空间: Iwork (max (nrow,ncol)) */
	    B2 = CORE(copy) (B, 0, values, Common) ;
	    if (Common->status < SPARSE_OK)
	    {
    ...
    nzmax = CORE(nnz) (A, Common) + CORE(nnz) (B, Common) ;

    C = CORE(allocate_sparse) (nrow, ncol, nzmax, FALSE, TRUE,
	    SIGN (A->stype), values ? A->xtype : SPARSE_PATTERN, Common) ;
    if (Common->status < SPARSE_OK)
    {
	CORE(free_sparse) (&A2, Common) ;
	CORE(free_sparse) (&B2, Common) ;
	return (NULL) ;	    /* 内存溢出 */
    }
    ...
    CORE(reallocate_sparse) (nz, C, Common) ;
...
)
{
	...
	C = CORE(allocate_sparse) (A->nrow, ncol, nz, sorted, TRUE,
	...
	CORE(reallocate_sparse) (nz, A, Common) ;
    }
...
)
{
    ...
    /* 给C分配空间.只有A有序，C才有序  */
    C = CORE(allocate_sparse) (nrow, ncol, nz + extra, A->sorted, TRUE, 0,
	    values ? A->xtype : SPARSE_PATTERN, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (NULL) ;
...
)
{
	...
	C = CORE(band) (A, -nrow, ncol, mode, Common) ;
	...
	{
	    /* C = triu (A) */
	    C = CORE(band) (A, 0, ncol, mode, Common) ;
	}
	else
	{
	    /* C = tril (A) */
	    C = CORE(band) (A, -nrow, 0, mode, Common) ;
	}
	if (Common->status < SPARSE_OK)
	{
	    /* 内存溢出 */
	...
	/* 从上到下或从下到上的转换 */
	/* 工作空间: Iwork (nrow) */
	C = CORE(transpose) (A, values, Common) ;
	if (!diag)
	{
	    /* 如果需要，去掉对角线 */
	    CORE(band_inplace) (-nrow, ncol, -1, C, Common) ;
	}
...
)
{
    ...
    s = CORE(add_size_t) (nrow, ((fset != NULL) ? ncol : 0), &ok) ;
    if (!ok)
    {
	ERROR (SPARSE_TOO_LARGE, "problem too large") ;
	return (FALSE) ;
...
)
{
    ...
    /* s = (Perm != NULL) ? 2*n : n */
    s = CORE(add_size_t) (n, ((Perm != NULL) ? n : 0), &ok) ;
    if (!ok)
    {
		ERROR (SPARSE_TOO_LARGE, "problem too large") ;
		return (FALSE) ;
...
{
    return (CORE(ptranspose) (A, values, NULL, NULL, 0, Common)) ;
}
...
)
{
    ...
    if (stype != 0)
    {
		...
		if (Perm != NULL)
		{
			ineed = CORE(mult_size_t) (A->nrow, 2, &ok) ;
		}
		else
		{
			ineed = A->nrow ;
    ...
    CORE(allocate_work) (0, ineed, 0, Common) ;
    if (Common->status < SPARSE_OK)
    {
		return (NULL) ;	    /* 内存溢出 */
    }
	 ...
	 */
    F = CORE(allocate_sparse) (ncol, nrow, fnz, TRUE, TRUE, -SIGN(stype),
	    xtype, Common) ;
    if (Common->status < SPARSE_OK)
    {
		return (NULL) ;	    /* 内存溢出 */
    ...
    if (stype != 0)
    {
		...
		ok = CORE(transpose_sym) (A, values, Perm, F, Common) ;
    }
    else
    {
		/* F = A (p,f)' */
		ok = CORE(transpose_unsym) (A, values, Perm, fset, nf, F, Common) ;
    }
...
)
{
    ...
    F = CORE(allocate_sparse) (ncol, nrow, anz, TRUE, TRUE, stype,
	    A->xtype, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (FALSE) ;	/* 内存溢出 */
    ...
    {
	/* F = A', 只用上三角或者下三角 */
	CORE(transpose_sym) (A, 1, NULL, F, Common) ;
	A->packed = TRUE ;
	/* A = F' */
	CORE(transpose_sym) (F, 1, NULL, A, Common) ;
    }
    else
    {
	/* F = A' */
	CORE(transpose_unsym) (A, 1, NULL, NULL, 0, F, Common) ;
	A->packed = TRUE ;
	/* A = F' */
	CORE(transpose_unsym) (F, 1, NULL, NULL, 0, A, Common) ;
    }
    ...
    Ap = A->p ;
    anz = Ap [ncol] ;
    CORE(reallocate_sparse) (anz, A, Common) ;
