...
)
{
    ...
    if (use_workspace)
    {
		CORE(allocate_work) (0, 0, nrow, Common) ;
		W = Common->Xwork ;
...
)
{
    ...
    if (A->stype && X->ncol >= 4)
    {
	w = CORE(malloc) (nx, 4*e*sizeof (double), Common) ;
    }
    if (Common->status < SPARSE_OK)
    {
	return (FALSE) ;    /* 内存溢出 */
    ...
    CORE(free) (4*nx, e*sizeof (double), w, Common) ;
