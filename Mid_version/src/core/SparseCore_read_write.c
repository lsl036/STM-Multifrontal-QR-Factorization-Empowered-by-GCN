...
)
{
    ...
    {
	return (CORE(allocate_triplet) (nrow, ncol, 0, 0, SPARSE_REAL,
		    Common)) ;
    }
    ...
    nnz2 = CORE(add_size_t) (nnz, extra, &ok) ;
    ...
    /* s = nrow + ncol */
    s = CORE(add_size_t) (nrow, ncol, &ok) ;
	...
	if (k == 0)
	{
	    ...
	    /* 给三元组矩阵分配空间 */
	    T = CORE(allocate_triplet) (nrow, ncol, nnz2, stype,
...
)
{
    ...
    {
	/* 返回一个空的稠密矩阵 */
	return (CORE(zeros) (nrow, ncol, SPARSE_REAL, Common)) ;
    }
 ...
 * 
 */
sparse_triplet *CORE(read_triplet)
(
    /* ---- input ---- */
    FILE *f,		/* 读取的文件，必须已经打开 */
    /* --------------- */
...
)
{
    ...
    T = CORE(read_triplet) (f, Common) ;
    A = CORE(triplet_to_sparse) (T, 0, Common) ;
    CORE(free_triplet) (&T, Common) ;

    if (Common->prefer_upper && A != NULL && A->stype == -1)
    {
	/* A=A' */
	A2 = CORE(transpose) (A, 2, Common) ;
