...
SparseQR_factorization *SparseQR(
    TPSM_t *tpool, int ordering, double tol,  
    sparse_csc *A, sparse_common *cc )
{
    ...
    qr_1colamd (ordering, tol, A, &Q1fill,
        &R1p, &P1inv, &Y, &n1cols, &n1rows, cc) ; //1.2
    ordering = cc->SPQR_istat [7]  ;
    // printf("qr_1colamd ordering = %d\n", ordering);
    ...
    if (noY) 
    {
        ...
        QRsym = qr_analyze (A, QR_ORDERING_GIVEN, Q1fill,tol >= 0, cc) ; //符号分析
        QRnum = qr_factorize (tpool, &A, FALSE, tol, n, QRsym, cc) ; //数值分解
    }
    else
    {
        QRsym = qr_analyze (Y, QR_ORDERING_FIXED, NULL, tol >= 0, cc) ; //符号分析
        QRnum = qr_factorize (tpool, &Y, TRUE, tol, n2, QRsym, cc) ;
    }
...
)
{
    ...
    *p_n1rows = EMPTY ;
    ...
    else
    {
        ...
        {
            //使用CHOL的内部排序(由cc定义)在
            cc->supernodal = SPARSE_SIMPLICIAL ;
            cc->postorder = TRUE ;
            sparse_factor *Sc ;
            Sc = SparseChol_analyze_p2 (FALSE, AT, NULL, NULL, 0, cc) ;
            if (Sc != NULL)
            {
        ...
        cc->SPQR_istat [7] = ordering ; // QR_ORDERING_COLAMD
        // 保存值
