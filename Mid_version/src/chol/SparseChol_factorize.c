...
)
{
    ...
    zero [0] = 0 ;
    zero [1] = 0 ;
    return (CHOL(factorize_p) (tpool, A, zero, NULL, 0, L, Common)) ;
}
...
)
{
    ...
    s = CORE(mult_size_t) (nsuper, 2, &ok) ;
    s = MAX (uncol, s) ;
    t = CORE(mult_size_t) (nrow, 2, &ok) ;
    s = CORE(add_size_t) (s, t, &ok) ;
    if (!ok)
    {
    ...
    CORE(allocate_work) (nrow, s, 0, Common) ;
	...
	if (L->ordering == SPARSE_NATURAL)
	{
		...
		/* S = tril (A'), 不需要F */
		/* 工作空间: Iwork (nrow) */
		A1 = CORE(ptranspose) (A, 2, NULL, NULL, 0, Common) ;
		S = A1 ;
	    }
	    else if (stype < 0)
	    {
		...
		A1 = CORE(ptranspose) (A, 2, NULL, fset, fsize, Common) ;
		F = A1 ;
		/* S = A */
		S = A ;
	    }
	...
	else
	{
	    ...
	    if (stype > 0)
	    {
			...
			/* S = tril (PAP'); 不需要F */
			/* 工作空间： Iwork (2*nrow) */
			A1 = CORE(ptranspose) (A, 2, L->Perm, NULL, 0, Common) ;
			S = A1 ;
	    }
	    else if (stype < 0)
	    {
		/* A2 = triu (PAP') */
		/* 工作空间： Iwork (2*nrow) */
		A2 = CORE(ptranspose) (A, 2, L->Perm, NULL, 0, Common) ;
		/* S = tril (A2'); 不需要F */
		/* 工作空间： Iwork (nrow) */
		A1 = CORE(ptranspose) (A2, 2, NULL, NULL, 0, Common) ;
		S = A1 ;
		...
		A1 = CORE(ptranspose) (A, 2, L->Perm, fset, fsize, Common) ;
		F = A1 ;
		/* S = F' */
		/* 工作空间： Iwork (nrow) */
		A2 = CORE(ptranspose) (F, 2, NULL, NULL, 0, Common) ;
		S = A2 ;
	    }
	}
	...
	{
		// F, beta 为空
	    CHOL(super_numeric) (tpool, S, F, beta, L, Common) ;
	}
	...
	{
	    /* 工作空间： none */
	    ok = CORE(change_factor) (L->xtype, Common->final_ll,
		...
		CHOL(resymbol_noperm) (S, fset, fsize, Common->final_pack,
		    L, Common) ;
	    }
	}
	...
	if (L->ordering == SPARSE_NATURAL)
	{
		...
		/* F不需要, S = A' */
		/* 工作空间： Iwork (nrow) */
		A2 = CORE(ptranspose) (A, 2, NULL, NULL, 0, Common) ;
		S = A2 ;
	    }
	    else
	    {
	...
	else
	{
	    ...
	    if (stype > 0)
	    {
			...
			/* A2 = triu (F') */
			/* 工作空间： Iwork (nrow) */
			A2 = CORE(ptranspose) (A1, 2, NULL, NULL, 0, Common) ;
			/* 对称情况不需要F，释放它并设为NULL*/
	...
	if (Common->status == SPARSE_OK)
	{
	    ...
	    CHOL(rowfac) (S, F, beta, 0, nrow, L, Common) ;
	    Common->grow2 = grow2 ;
	}
	status = Common->status ;
	...
	{
	    /* 工作空间： none */
	    CORE(change_factor) (L->xtype, L->is_ll, FALSE,
...
)
{
    ...
    s = CORE(mult_size_t) (nrow, 2, &ok) ;
    s = CORE(add_size_t) (s, (stype ? 0 : ncol), &ok) ;
    if (!ok)
    {
	ERROR (SPARSE_TOO_LARGE, "problem too large") ;
	return (FALSE) ;
	...
	if (L->ordering == SPARSE_NATURAL)
	{
	    /* F = triu(A)' */
	    /* 工作空间： Iwork (nrow) */
	    G = CORE(ptranspose) (A, 0, NULL, NULL, 0, Common) ;
	}
	else
	{
	    /* F = triu(A(p,p))' */
	    /* 工作空间： Iwork (2*nrow) */
	    G = CORE(ptranspose) (A, 0, L->Perm, NULL, 0, Common) ;
	}
	F = G ;
    }
    else if (stype < 0)
	...
	else
	{
	    ...
	    /* H = G' */
	    /* 工作空间： Iwork (nrow) */
	    H = CORE(ptranspose) (G, 0, NULL, NULL, 0, Common) ;
	    F = H ;
	}
    }
    else
	...
	else
	{
	    /* G = A(p,f)' */
	    /* 工作空间： Iwork (nrow if no fset; MAX (nrow,ncol) if fset)*/
	    G = CORE(ptranspose) (A, 0, L->Perm, fset, fsize, Common) ;
	    /* H = G' */
	    /* 工作空间： Iwork (ncol) */
	    H = CORE(ptranspose) (G, 0, NULL, NULL, 0, Common) ;
	    F = H ;
    ...
    ok = CHOL(resymbol_noperm) (F, fset, fsize, pack, L, Common) ;

    /* ---------------------------------------------------------------------- */
    /* 释放临时矩阵，如果它们存在的话 */
    /* ---------------------------------------------------------------------- */

    CORE(free_sparse) (&H, Common) ;
    CORE(free_sparse) (&G, Common) ;
    return (ok) ;
}
...
)
{
	...
	/* 把L缩小到足够大。它不能失败。 */
	/* 工作空间： none */
	CORE(reallocate_factor) (Lp [nrow], L, Common) ;
    }
