...
)
{
    ...
    /* 求解，根据需要分配工作空间  */
    ok = CHOL(solve2) (sys, L, B, NULL, &X, NULL, &Y, &E, Common) ;

    /* 如果分配了工作区，则释放;如果发生错误，则释放结果 */
    CORE(free_dense) (&Y, Common) ;
    CORE(free_dense) (&E, Common) ;
    if (!ok)
    {
...
)
{
    ...
    /* 确保X有正确的大小和类型 */
    X = CORE(ensure_dense) (X_Handle, n, nrhs, n, xtype, Common) ; //在hnucore_matrix_type.c 里面
    ...
    if (Bset)
    {
        ...
        if (L->is_super)
        {
             ...
             */
            CORE(change_factor) (   // 在 hnucore_change_factor.c 里面
        ...
        /* L, X, B是相同的数据类型*/
        /* 确保Y的大小正确 */
	Y = CORE(ensure_dense) (Y_Handle, 1, n, 1, L->xtype, Common) ;
	if (Common->status < SPARSE_OK)
	{
	    /* 内存溢出 */
	    return (FALSE) ;
        ...
        if ((sys == SPARSE_A || sys == SPARSE_P) && Perm != NULL)
        {
            ...
            if (L->IPerm == NULL)
            {
                ...
                L->IPerm = CORE(malloc) (n, sizeof (Int), Common) ;
                if (Common->status < SPARSE_OK)
                {
                    /* 内存溢出 */
                    return (FALSE) ;
            ...
            (Int) Xset->nzmax < n || Xset->itype != SPARSE_PATTERN)
        {
            ...
            CORE(free_sparse) (Xset_Handle, Common) ;
            Xset = CORE(allocate_sparse) (n, 1, n, FALSE, TRUE, 0,
        ...
        CORE(allocate_work) (n, 3*n, 0, Common) ;
        if (Common->status < SPARSE_OK)
        {
            /* 内存溢出 */
            return (FALSE) ;
        ...
        {
            if (!CHOL(lsolve_pattern) (C, L, Yset, Common)) //SparseChol_analyze.c中
            {
	...
	Y = CORE(ensure_dense) (Y_Handle, n, dual*nrhs, n, L->xtype, Common);
	E = CORE(ensure_dense) (E_Handle, dual*nrhs, L->maxesize, dual*nrhs,
		L->xtype, Common) ;
	...
	{
	    CHOL(super_lsolve) (L, Y, E, Common) ;	    /* Y = L\Y */
            CHOL(super_ltsolve) (L, Y, E, Common) ;	    /* Y = L'\Y*/
	}
	...
	Y = CORE(ensure_dense) (Y_Handle, nr, n, nr, ytype, Common) ;
	if (Common->status < SPARSE_OK)
	{
	    /* 内存溢出 */
	    return (FALSE) ;
