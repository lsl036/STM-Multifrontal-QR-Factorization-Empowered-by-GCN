...
// 数值分解的线程池并行函数
void* parallel_numeric_factor(chol_snode_args *snode_args)
{   
    int s = snode_args->snode_id;
    int nsuper = snode_args->base_args->nsuper;
    int repeat_supernode = snode_args->base_args->repeat_supernode;
...
// #pragma omp parallel for num_threads(SPARSE_OMP_NUM_THREADS) \
//     if ( n > 128 ) schedule (static)
    ...
    for(i = max_depth ; i >= 0; --i)
    {
        ...
        {
        #ifdef PTHREAD_POOL
            if(TPSM_addTask(tpool, &parallel_numeric_factor, &snode_args[j], 1, 0)==1)
            {
...
)
{
    ...
    /* w = 2*n + 5*nsuper */
    w = CORE(mult_size_t) (n, 2, &ok) ;
    t = CORE(mult_size_t) (nsuper, 5, &ok) ;
    w = CORE(add_size_t) (w, t, &ok) ;
    if (!ok)
    {
    ...
    CORE(allocate_work) (n, w, 0, Common) ;
    if (Common->status < SPARSE_OK)
    {
	    return (FALSE) ;
    }
    ...
    {
        /* 通过分配L->x转换到超节点数值 */
        CORE(change_factor) ( SPARSE_REAL, TRUE, TRUE, TRUE, TRUE, L, Common) ;
        if (Common->status < SPARSE_OK)
        {
            /* 因子L保持符号超节点形式 */
            return (FALSE) ;
    ...
    C = CORE(allocate_dense) (maxcsize, 1, maxcsize, L->xtype, Common) ;
    if (Common->status < SPARSE_OK)
    {
        ...
        if (symbolic)
        {
            ...
            CORE(change_factor) (SPARSE_PATTERN, TRUE, TRUE, TRUE, TRUE,
                L, Common) ;
        }
        /* 因子L现在回到了它在输入时的形式 */
    ...
    {
        ok = r_SparseChol_super_numeric (tpool, A, F, beta, L, C, Common) ;
    }
