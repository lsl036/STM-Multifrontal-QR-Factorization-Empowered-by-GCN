...
)
{
    ...
    *F_handle = NULL ;
	...
	if (A->stype < 0)
	{
	    ...
	    A2 = CORE(ptranspose) (A, 0, NULL, NULL, 0, Common) ;
	    F = A ;
	    S = A2 ;
	}
	else if (A->stype > 0)
	{
		...
		A1 = CORE(ptranspose) (A, 0, NULL, fset, fsize, Common) ;
	    }
	    F = A1 ;
	    S = A ;
	}
	...
	if (A->stype < 0)
	{
	    ...
	    A2 = CORE(ptranspose) (A, 0, Perm, NULL, 0, Common) ;
	    S = A2 ;
	    /* workspace: Iwork (nrow) */
	    if (do_rowcolcounts)
	    {
		/* 如果do_rowcolcounts为FALSE，则在对称情况下不需要F */
		A1 = CORE(ptranspose) (A2, 0, NULL, NULL, 0, Common) ;
	    }
	    F = A1 ;
	}
	else if (A->stype > 0)
	{
	    ...
	    /* workspace: Iwork (2*nrow) */
	    A1 = CORE(ptranspose) (A, 0, Perm, NULL, 0, Common) ;
	    F = A1 ;
	    /* workspace: Iwork (nrow) */
	    A2 = CORE(ptranspose) (A1, 0, NULL, NULL, 0, Common) ;
	    S = A2 ;
	}
	else
	{
	    ...
	    /* workspace: Iwork (nrow if no fset, MAX(nrow,ncol) if fset) */
	    A1 = CORE(ptranspose) (A, 0, Perm, fset, fsize, Common) ;
	    F = A1 ;
	    if (do_rowcolcounts)
	    {
...
)
{
    ...
    do_rowcolcounts = (ColCount != NULL) ; //  = 1
    ...
    ok = ok && CHOL(etree) (A->stype ? S:F, Parent, Common) ; //(S, Parent, Common)

    /* 后序遍历消去树（由SparseCore_rowcolcounts要求） */
    /* workspace: Iwork (2*nrow) */
    ok = ok && (CHOL(postorder) (Parent, n, NULL, Post, Common) == n) ;
    ...
    if (do_rowcolcounts)
    {
	ok = ok && CHOL(rowcolcounts) (A->stype ? F:S, fset, fsize, Parent,
	    Post, NULL, ColCount, First, Level, Common) ;
    }

    /* 释放临时矩阵和返回结果 */
    CORE(free_sparse) (&A1, Common) ;
    CORE(free_sparse) (&A2, Common) ;
    return (ok) ;
}
...
{ \
    ...
    CORE(free) (n, sizeof (Int), Lparent,  Common) ; \
    CORE(free) (n, sizeof (Int), Perm,     Common) ; \
    CORE(free) (n, sizeof (Int), ColCount, Common) ; \
...
{
    return (CHOL(analyze_p2) (TRUE, A, NULL, NULL, 0, Common)) ;
}
...
)
{
	...
	*Post, *Perm, *Lparent, *Lcolcount ;
    ...
    nmethods = MAX (0, nmethods) ; // nmethods=0

    default_strategy = (nmethods == 0) ;
    if (default_strategy)
    {
        /* 默认策略：如果提供，请尝试UserPerm。对A尝试使用AMD，或使用AMD对A * A'排序 */
        Common->method [0].ordering = SPARSE_GIVEN ;/* skip if UserPerm NULL */
        Common->method [1].ordering = SPARSE_AMD ;
        amd_backup = FALSE ;
        nmethods = 2 ; // nmethods = 2
    }
    else
    {
...
#ifdef NSUPERNODAL
    /* 未安装HNUCHOL Supernodal模块，只需做简单分析 */
    Common->supernodal = SPARSECHOL_SIMPLICIAL ;
#endif
    ...
    /* s = 6*n + uncol */
    s = CORE(mult_size_t) (n, 6, &ok) ;
    s = CORE(add_size_t) (s, uncol, &ok) ;
    if (!ok)
    {
    ...
    CORE(allocate_work) (n, s, 0, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (NULL) ;	    /* 内存溢出 */
    }
    ...
    L = CORE(allocate_factor) (n, Common) ;
    Lparent  = CORE(malloc) (n, sizeof (Int), Common) ;
    Perm     = CORE(malloc) (n, sizeof (Int), Common) ;
    ColCount = CORE(malloc) (n, sizeof (Int), Common) ;
    if (Common->status < SPARSE_OK)
    {
	/* 内存溢出 */
    ...
    for (method = 0 ; method <= nmethods ; method++) // method=0: GIVEN; method=1:AMD
    {
        /* ------------------------------------------------------------------ */
        /* 确定尝试的方法 */
        /* ------------------------------------------------------------------ */
        ...
        else if (ordering == SPARSE_AMD)
        {
            /* -------------------------------------------------------------- */
            /* A，A * A'或AN（：，f）* A（：，f）'的AMD顺序 */
            /* -------------------------------------------------------------- */
            amd_backup = FALSE ;    /* 不需要尝试AMD两次 */
            CHOL(amd) (A, fset, fsize, Perm, Common) ; // Cholesky 走这里
            skip_analysis = TRUE ;
        ...
        else if (ordering == SPARSE_COLAMD)
        {
            ...
            if (A->stype)
            {
                CHOL(amd) (A, fset, fsize, Perm, Common) ;
                skip_analysis = TRUE ;
            }
            else
            {
                ...
                CHOL(colamd) (A, fset, fsize, FALSE, Perm, Common) ;
            }
        ...
        if (!skip_analysis) // Cholesky不走这里
        {
            if (!CHOL(analyze_ordering) (A, ordering, Perm, fset, fsize,
    ...
    if (skip_best) //TRUE
    {
        if (!CHOL(analyze_ordering) (A, L->ordering, Lperm, fset, fsize,
    ...
    if (Common->postorder) // TRUE
    {
        /* 结合减少填充的排序和加权的后序遍历 */
        /* workspace: Iwork (2*nrow) */
        if (CHOL(postorder) (Lparent, n, Lcolcount, Post, Common) == n)
        {
            /* 使用First和Level作为工作空间 */
	...
	CHOL(super_symbolic2) (for_whom, S, F, Lparent, L, Common) ;
...
)
{
	...
	C = CORE(aat) (A, fset, fsize, -2, Common) ;
    }
    else // 对称
    {
     ...
     * 向C添加nnz（C）/ 2 + n的额外空间 */
	C = CORE(copy) (A, 0, -2, Common) ; // 仅拷贝非零元分布模式
    }
...
(
    ...
    Int *fset,		    /* 0:(A->ncol)-1的子集 */
    size_t fsize,	    /* fset的大小 */
    int postorder,	    /* 如果为TRUE，请遵循coletree的后序遍历 */
    /* ---- output --- */
    Int *Perm,		    /* size A->nrow, output permutation */
    /* --------------- */
    sparse_common *Common
)
{
    ...
    /* s = 4*nrow + ncol */
    s = CORE(mult_size_t) (nrow, 4, &ok) ;
    s = CORE(add_size_t) (s, ncol, &ok) ;
    ...
    CORE(allocate_work) (0, s, 0, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (FALSE) ;
    }
    ...
    C = CORE(allocate_sparse) (ncol, nrow, alen, TRUE, TRUE, 0,
    ...
    /* C = A (:,f)',如果需要可以打包A */
    /* workspace: Iwork (nrow if no fset; MAX (nrow,ncol) if fset) */
    ok = CORE(transpose_unsym) (A, 0, NULL, fset, fsize, C, Common) ;
	...
	ok = ok && CHOL(analyze_ordering) (A, SPARSE_COLAMD, Perm, fset,
...
)
{
    ...
    s = CORE(add_size_t) (A->nrow, (stype ? 0 : A->ncol), &ok) ;
    if (!ok)
    {
	ERROR (SPARSE_TOO_LARGE, "problem too large") ;
	return (FALSE) ;
 ...
 * @return Sparse_long 
 */
Sparse_long CHOL(postorder)	/* 返回后序遍历节点的# */
(
    /* ---- input ---- */
...
)
{
	...
	*Iwork ;
    ...
    w = CORE(mult_size_t) (nrow, 2, &ok) ;
    w = CORE(add_size_t) (w, (stype ? 0 : ncol), &ok) ;
    if (!ok)
    {
	ERROR (SPARSE_TOO_LARGE, "problem too large") ;
	return (FALSE) ;
    ...
    CORE(allocate_work) (nrow, w, 0, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (FALSE) ;
    }
...
)
{
    ...
    nrow = A->nrow ;
    CORE(allocate_work) (nrow, 0, 0, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (FALSE) ;
    }
...
)
{
    ...
    krow = B->nrow ;
    return (CHOL(row_lsubtree) (B, NULL, 0, krow, L, Yset, Common)) ;
}
...
{
    return (CHOL(rowfac_mask2) (A, F, beta, kstart, kend, NULL, 0, NULL, L,
	Common)) ;
}
...
{
    Int maskmark = 0 ;
    return (CHOL(rowfac_mask2) (A, F, beta, kstart, kend, mask, maskmark,
        RLinkUp, L, Common)) ;
}
...
)
{
    ...
    s = CORE(mult_size_t) (n, ((A->xtype != SPARSE_REAL) ? 2:1), &ok) ;
    if (!ok)
    {
	ERROR (SPARSE_TOO_LARGE, "problem too large") ;
	return (FALSE) ;
    ...
    CORE(allocate_work) (n, n, s, Common) ;
    if (Common->status < SPARSE_OK)
    {
	return (FALSE) ;
    }
